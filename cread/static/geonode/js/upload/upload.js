...
define(['underscore',
    ...
    buildFileInfo = function (files) {
        ...
        for (name in files) {	
			...
			if (files.hasOwnProperty(name) && _continue) {
    ...
    initialize = function (options) {
		...
		$( document ).on( "layerRowRemoved", function( event, param ) {
			
			
			// ////////////////////////////////////////////////////////////////
			// TODO HERE: clean all the other stuff in the form:
			//            - layer name beside the choice button
			//            - record title
			//            - any other html fragment or JS variable related to 
			//              the currently removed file. 
			// /////////////////////////////////////////////////////////////////
			if(param.files.length < 1){
				/*delete layers[param.name];	
				
				if(jQuery.isEmptyObject(layers)){
					$( "#" + param.name + "-element").remove();
				}*/
				doClearState();
			}
